...
public class R2dbcExpenditureRepository implements ExpenditureRepository {
    ...
    // jdbcTemplateみたいなもの　jdbcTemplateより高機能
    ...
    public Mono<Expenditure> findById(Integer expenditureId) {
        ...
        return this.databaseClient
                .select().from(Expenditure.class)
                .matching(where("expenditureId").is(expenditureId))
                .as(Expenditure.class)
                .one();

//        this.databaseClient.execute()
//                .sql("select expedituerId, expenduire_name, uniprice from expediture where expirenreid = :id")
//                .bind("id", expenditureId)
//                .map((row, rowMetadata)
//
//                )
//                //.as(Expenditure.class)
